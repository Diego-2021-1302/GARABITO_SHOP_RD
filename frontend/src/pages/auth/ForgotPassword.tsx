...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSent(true);
      addNotification('success', 'Enlace de recuperación enviado a tu correo');
    } catch (error: any) {
      addNotification('error', error.response?.data?.message || 'Error al enviar el correo');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white flex items-center justify-center p-6 relative overflow-hidden font-poppins">
      <SEO title="Recuperar Contraseña | Garabito Shop RD" />
      ...
      >
        <div className="bg-[#020617]/80 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-10 text-center">
            ...
            <h1 className="text-2xl font-black tracking-tight mb-2 uppercase">Recuperar Acceso</h1>
            <p className="text-gray-500 text-xs font-medium max-w-xs uppercase tracking-widest leading-relaxed">
              {isSent 
          ...
          {!isSent ? (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="block w-full pl-11 pr-4 py-4 bg-white/[0.03] border border-white/10 rounded-2xl text-[11px] font-medium transition-all focus:ring-1 focus:ring-brand-primary/50 focus:border-brand-primary/50 focus:bg-white/[0.05] outline-none placeholder:text-gray-700 text-white"
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full bg-brand-primary text-white py-4 rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-brand-primary/20 flex items-center justify-center gap-3 disabled:opacity-50"
              >
                {isLoading ? (
            ...
            <div className="text-center py-4">
              ...
              <p className="text-sm text-gray-400">Si el correo existe en nuestro sistema, recibirás las instrucciones en breve.</p>
            </div>
          )}

          <div className="mt-10 pt-8 border-t border-white/5 text-center">
            <Link to="/login" className="inline-flex items-center gap-2 text-gray-500 text-[10px] font-black uppercase tracking-widest hover:text-brand-primary transition-all">
